...
class Cid:
    ...
    def aws_login(self):
        params = {"profile_name": None, "region_name": None, "aws_access_key_id": None, "aws_secret_access_key": None, "aws_session_token": None}
        ...
        try:
            ...
            self.qs_url_params = {"account_id": self.base.account_id, "region": self.base.session.region_name}
    ...
    def qs(self) -> QuickSight:
        if not self._clients.get("quicksight"):
            self._clients.update({"quicksight": QuickSight(self.base.session, resources=self.resources)})
        return self._clients.get("quicksight")

    @property
    def organizations(self) -> Organizations:
        if not self._clients.get("organizations"):
            self._clients.update({"organizations": Organizations(self.base.session)})
        return self._clients.get("organizations")

    @property
    def s3(self) -> S3:
        if not self._clients.get("s3"):
            self._clients.update({"s3": S3(self.base.session)})
        return self._clients.get("s3")

    @property
    def athena(self) -> Athena:
        if not self._clients.get("athena"):
            self._clients.update({"athena": Athena(self.base.session, resources=self.resources)})
        return self._clients.get("athena")

    @property
    def glue(self) -> Glue:
        if not self._clients.get("glue"):
            self._clients.update({"glue": Glue(self.base.session)})
        return self._clients.get("glue")

    @property
    def iam(self) -> Glue:
        if not self._clients.get("iam"):
            self._clients.update({"iam": IAM(self.base.session)})
    ...
    def accountMap(self) -> AccountMap:
        if not self._clients.get("accountMap"):
            ...
            self._clients.update({"accountMap": _account_map})
    ...
    def command(func):
        ...
        def wrap(self, *args, **kwargs):
            ...
            if not self._logger:
                self._logger = set_cid_logger(verbosity=self.verbose, log_filename=get_parameters().get("log_filename", "cid.log"))
    ...
    def __loadPlugins(self) -> dict:
        ...
        for ep in _entry_points:
            ...
            try:
                self.resources = always_merger.merge(self.resources, plugin.provides())
    ...
    def track(self, action, dashboard_id):
        ...
        try:
            res = requests.request(method=method, url=endpoint, data=json.dumps(payload), headers={"Content-Type": "application/json"})
            if res.status_code != 200:
                logger.debug(
                    f"This will not fail the deployment. There has been an issue logging action {action}  for dashboard {dashboard_id} and account {self.base.account_id}, server did not respond with a 200 response,actual  status: {res.status_code}, response data {res.text}. This issue will be ignored"
                )
    ...
    def _deploy(self, dashboard_id: str = None, recursive=True, update=False, **kwargs):
        ...
        if get_yesno_parameter(param_name=f"share-with-account", message=f"Share this dashboard with everyone in the account?", default="yes"):
    ...
    def open(self, dashboard_id, **kwargs):
        ...
        if dashboard is not None:
            if dashboard.version.get("Status") not in ["CREATION_SUCCESSFUL"]:
                print(json.dumps(dashboard.version.get("Errors"), indent=4, sort_keys=True, default=str))
        ...
        for dataset in list(self.qs._datasets.values()) if self.qs._datasets else []:
            if dataset.id == id or dataset.name == name:
                ...
                for dashboard in (self.qs.dashboards or {}).values():
                    ...
                    if (
                        get_parameter(
                            param_name=f"confirm-{dataset.name}", message=f"Delete QuickSight Dataset {dataset.name}?", choices=["yes", "no"], default="no"
                        )
                        == "yes"
                    ):
    ...
    def _share(self, dashboard_id, **kwargs):
        ...
        if share_method == "folder":
            folder = None
            folder_methods = {"Select Existing folder": "existing", "Create New folder": "new"}
            ...
            if folder_method == "existing":
                try:
                    ...
                    while not folder:
                        folder_id = get_parameter(param_name="folder-id", message="Please enter the folder Id to use")
                ...
                while not folder:
                    try:
                        folder_name = get_parameter(param_name="folder-name", message="Please enter the folder name to create")
                        ...
                        columns_tpl = {"PrincipalArn": self.qs.get_principal_arn()}
            ...
            if share_method == "account":
                ...
                while not user:
                    user_name = get_parameter(param_name="quicksight-user", message="Please enter the user name to share with")
            ...
            columns_tpl = {"PrincipalArn": principal_arn}
            ...
            dashboard_params = {"GrantPermissions": [dashboard_permissions]}
            if share_method == "account":
                dashboard_params.update({"GrantLinkPermissions": [dashboard_permissions]})
    ...
    def update_dashboard(self, dashboard_id, recursive=False, **kwargs):
        ...
        if dashboard.status == "legacy":
            if (
                get_parameter(param_name=f"confirm-update", message=f"Dashboard template changed, update it anyway?", choices=["yes", "no"], default="yes")
                != "yes"
            ):
                return
        elif dashboard.latest:
            if (
                get_parameter(param_name=f"confirm-update", message=f"No updates available, should I update it anyway?", choices=["yes", "no"], default="yes")
                != "yes"
            ):
                return
    ...
    def create_datasets(self, _datasets: list, known_datasets: dict = {}, recursive: bool = True, update: bool = False) -> dict:
        ...
        if len(missing_datasets):
            ...
            while len(missing_datasets):
                ...
                _id = get_parameter(param_name=f"{dataset_name}-dataset-id", message=f"DataSetId/Arn for {dataset_name}")
                id = _id.split("/")[-1]
                try:
    ...
    def get_dataset_data_from_definition(self, dataset_definition):
        ...
        if dataset_file:
            raw_template = json.loads(resource_string(dataset_definition.get("providedBy"), f"data/datasets/{dataset_file}").decode("utf-8"))
