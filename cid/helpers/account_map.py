...
from cid.helpers import CUR, Athena
...
class AccountMap(CidBase):
    defaults = {"MetadataTableNames": ["acc_metadata_details", "organisation_data"]}
    _clients = dict()
    _accounts = list()
    ...
    def athena(self) -> Athena:
        if not self._clients.get("athena"):
            self._clients.update({"athena": Athena(self.session)})
        return self._clients.get("athena")

    @athena.setter
    def athena(self, client) -> Athena:
        if not self._clients.get("athena"):
            self._clients.update({"athena": client})
        return self.athena
    ...
    def cur(self, client) -> CUR:
        if not self._clients.get("cur"):
            self._clients.update({"cur": client})
        return self.cur
    ...
    def accounts(self) -> dict:
        if self._accounts:
            ...
            key_mapping = {"accountid": "account_id", "name": "account_name"}
    ...
    def create(self, name) -> bool:
        ...
        try:
            ...
            if self._AthenaTableName:
                ...
                # Fill in TPLs
                columns_tpl = dict()
                parameters = {"metadata_table_name": self._AthenaTableName, "cur_table_name": self.cur.tableName}
    ...
    def get_dummy_account_mapping_sql(self, name) -> list:
        ...
        # Fill in TPLs
        columns_tpl = dict()
        parameters = {"athena_view_name": name, "cur_table_name": self.cur.tableName}
        columns_tpl.update(**parameters)
        compiled_query = template.safe_substitute(columns_tpl)
    ...
    def get_csv_accounts(self, file_path) -> list:
        ...
        with open(file_path) as f:
            accounts = [{str(k).lower().replace(" ", "_"): str(v) for k, v in row.items()} for row in csv.DictReader(f, skipinitialspace=True)]
    ...
    def create_account_mapping_sql(self, name) -> str:
        ...
        for account in self.accounts:
            ...
            accounts_sql.append(
                """ROW ('{account_id}', '{account_name}:{account_id}', '{parent_account_id}', '{account_status}', '{account_email}')""".format(
                    account_name=account_name, **acc
                )
            )
        # Fill in TPLs
        columns_tpl = {"athena_view_name": name, "rows": ",".join(accounts_sql)}
        compiled_query = template.safe_substitute(columns_tpl)
