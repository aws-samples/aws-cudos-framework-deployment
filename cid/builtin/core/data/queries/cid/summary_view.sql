...
python3 ./cid/render_template.py ./cid/builtin/core/data/queries/cid/summary_view.sql \
...
CREATE OR REPLACE VIEW summary_view AS
 ...
 ,  CASE
       WHEN ("date_trunc"('month',"line_item_usage_start_date")) >= ("date_trunc"('month', current_timestamp) - INTERVAL  '3' MONTH) THEN "date_trunc"('day', "line_item_usage_start_date") 
...
%endif
%if not cur_has_reservations and not cur_has_reservations:
...
%endif
 ...
 , CASE
     WHEN (("line_item_usage_type" LIKE '%Spot%') AND ("line_item_product_code" = 'AmazonEC2') AND ("line_item_line_item_type" = 'Usage')) THEN "split_part"("line_item_line_item_description", '.', 1)
 ...
 END "instance_type"
 , CASE
     WHEN (("line_item_usage_type" LIKE '%Spot%') AND ("line_item_product_code" = 'AmazonEC2') AND ("line_item_line_item_type" = 'Usage')) THEN "split_part"("split_part"("line_item_line_item_description", ' ', 2), '/', 1)
     ELSE "product_operating_system"
 END "platform"
