...
FROM
  ((
   ...
   , (CASE WHEN ("savings_plan_savings_plan_a_r_n" <> '') THEN COALESCE(TRY(from_iso8601_timestamp("savings_plan_end_time")), "savings_plan_end_time") WHEN ("reservation_reservation_a_r_n" <> '') THEN COALESCE(TRY(from_iso8601_timestamp("reservation_end_time")), "savings_plan_end_time") ELSE null END) "ri_sp_end_date"
