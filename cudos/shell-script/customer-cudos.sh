...
aws quicksight create-dashboard --cli-input-json "file://${deployFileName}" --output table
...
####### "
    ...
    aws quicksight create-ingestion --ingestion-id `date +%Y_%m_%H_%M` --aws-account-id ${account} --data-set-id d01a936f-2b8f-49dd-8f95-d9c7130c5e46
    aws quicksight create-ingestion --ingestion-id `date +%Y_%m_%H_%M` --aws-account-id ${account} --data-set-id 826896be-4d0f-4f90-832f-3427f5444016
    aws quicksight create-ingestion --ingestion-id `date +%Y_%m_%H_%M` --aws-account-id ${account} --data-set-id 3fa0d804-9bf5-4a20-a61d-4bdbb6d543b1
    aws quicksight create-ingestion --ingestion-id `date +%Y_%m_%H_%M` --aws-account-id ${account} --data-set-id 9497cc49-c9b1-4dcd-8bcc-c16396898f29
...
updated_dashboard_versionArn=$(aws quicksight update-dashboard --cli-input-json "file://${updateFileName}" --query 'VersionArn')
...
if [ "${ahq_ri_query_result}" -gt 1 ] && [ "${ahq_sp_query_result}" -gt 1 ]; then
    ...
    aws quicksight describe-dashboard --dashboard-id $dashboardId --aws-account-id $account --output table
...
#    aws quicksight delete-dashboard --dashboard-id $dashboardId --aws-account-id $account --output table
    aws quicksight delete-data-source --aws-account-id $account --data-source-id 95aa6f18-abb4-436f-855f-182b199a961f
    aws quicksight delete-data-set --aws-account-id $account --data-set-id d01a936f-2b8f-49dd-8f95-d9c7130c5e46
    aws quicksight delete-data-set --aws-account-id $account --data-set-id 826896be-4d0f-4f90-832f-3427f5444016
    aws quicksight delete-data-set --aws-account-id $account --data-set-id 3fa0d804-9bf5-4a20-a61d-4bdbb6d543b1
    aws quicksight delete-data-set --aws-account-id $account --data-set-id 9497cc49-c9b1-4dcd-8bcc-c16396898f29
    aws quicksight delete-data-set --aws-account-id $account --data-set-id 595c66b7-08b6-46ad-87ed-b74fe34dd333
