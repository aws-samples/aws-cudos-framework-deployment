...
select
    ...
    , sum((CASE WHEN (line_item_line_item_type = 'SavingsPlanCoveredUsage') THEN savings_plan_savings_plan_effective_cost WHEN (line_item_line_item_type = 'DiscountedUsage') THEN reservation_effective_cost WHEN (line_item_line_item_type = 'Usage') THEN line_item_unblended_cost ELSE 0 END)) amortized_cost
    , round(sum(line_item_usage_amount), 2) usage_quantity
from cur2
WHERE (bill_billing_period_start_date >= (date_trunc('month', current_timestamp) - INTERVAL '1' MONTH)) AND (line_item_product_code = 'AmazonEC2') AND (product_servicecode <> 'AWSDataTransfer') AND (line_item_operation LIKE '%RunInstances%') AND (NOT (line_item_usage_type LIKE '%DataXfer%')) AND ((line_item_line_item_type = 'Usage') OR (line_item_line_item_type = 'SavingsPlanCoveredUsage') OR (line_item_line_item_type = 'DiscountedUsage'))
