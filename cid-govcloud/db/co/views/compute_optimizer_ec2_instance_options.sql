...
(
   SELECT
     COALESCE(TRY(date_parse(lastrefreshtimestamp_utc, '%Y-%m-%d %H:%i:%s')), TRY(date_parse(lastrefreshtimestamp_utc, '%m/%d/%y %H:%i'))) lastrefreshtimestamp_utc
   , accountid accountid
   , instancearn arn
   ...
   , finding finding
  --  this is specific to current, reasons for options 1-3 are empty strings
   , CONCAT((CASE WHEN (findingreasoncodes_iscpuoverprovisioned = 'true') THEN 'CPU-Over ' ELSE '' END), (CASE WHEN (findingreasoncodes_iscpuunderprovisioned = 'true') THEN 'CPU-Under ' ELSE '' END), (CASE WHEN (findingreasoncodes_isdiskiopsoverprovisioned = 'true') THEN 'DiskIOPS-Over ' ELSE '' END), (CASE WHEN (findingreasoncodes_isdiskiopsunderprovisioned = 'true') THEN 'DiskIOPS-Under ' ELSE '' END), (CASE WHEN (findingreasoncodes_isdiskthroughputoverprovisioned = 'true') THEN 'DiskThroughput-Over ' ELSE '' END), (CASE WHEN (findingreasoncodes_isdiskthroughputunderprovisioned = 'true') THEN 'DiskThroughput-Under ' ELSE '' END), (CASE WHEN (findingreasoncodes_isebsiopsoverprovisioned = 'true') THEN 'EBSIOPS-Over ' ELSE '' END), (CASE WHEN (findingreasoncodes_isebsiopsunderprovisioned = 'true') THEN 'EBSIOPS-Under ' ELSE '' END), (CASE WHEN (findingreasoncodes_isebsthroughputoverprovisioned = 'true') THEN 'EBSThroughput-Over ' ELSE '' END), (CASE WHEN (findingreasoncodes_isebsthroughputunderprovisioned = 'true') THEN 'EBSThroughput-Under ' ELSE '' END), (CASE WHEN (findingreasoncodes_ismemoryoverprovisioned = 'true') THEN 'Memory-Over ' ELSE '' END), (CASE WHEN (findingreasoncodes_ismemoryunderprovisioned = 'true') THEN 'Memory-Under ' ELSE '' END), (CASE WHEN (findingreasoncodes_isnetworkbandwidthoverprovisioned = 'true') THEN 'NetworkBandwidth-Over ' ELSE '' END), (CASE WHEN (findingreasoncodes_isnetworkbandwidthunderprovisioned = 'true') THEN 'NetworkBandwidth-Under ' ELSE '' END), (CASE WHEN (findingreasoncodes_isnetworkppsoverprovisioned = 'true') THEN 'NetworkPPS-Over ' ELSE '' END), (CASE WHEN (findingreasoncodes_isnetworkppsunderprovisioned = 'true') THEN 'NetworkPPS-Under ' ELSE '' END)) reason
   , lookbackperiodindays lookbackperiodindays
   , currentperformancerisk currentperformancerisk
   , errorcode errorcode
   , errormessage errormessage

 --  The following two are specific to current, ressouce_details and utilizationmetrics for options 1-3 are empty strings
   , CONCAT(effectiverecommendationpreferencesenhancedinfrastructuremetrics, ';', inferredworkloadtypes, ';') ressouce_details
...
--  The option_to and currency are for target recommendations, i.e., options 1-3
   , '' option_to
   , '' currency

--  The following four are specific to current, monthlyprice, hourlyprice, estimatedmonthlysavings_value, and estimatedmonthly_ondemand_cost_change are specific to each option
...
--  The option_details is specific to current and each option
   , CONCAT(COALESCE(currentperformancerisk, 'na'), ';', COALESCE(currentinstancetype, 'na'), ';', COALESCE('', 'na'), ';', COALESCE(current_memory, 'na'), ';', COALESCE(current_vcpus, 'na'), ';', COALESCE(current_network, 'na'), ';', COALESCE(current_storage, 'na'), ';', COALESCE('', 'na'), ';', COALESCE(utilizationmetrics_cpu_maximum, 'na'), ';', COALESCE(utilizationmetrics_memory_maximum, 'na'), ';') option_details
   , tags tags
   FROM
     compute_optimizer_ec2_instance_lines
   WHERE ((instancearn LIKE '%arn:%')
   AND (recommendationoptions_1_estimatedmonthlysavings_currency = '') and (recommendationoptions_2_estimatedmonthlysavings_currency = '') and (recommendationoptions_3_estimatedmonthlysavings_currency = ''))
UNION    SELECT
     COALESCE(TRY(date_parse(lastrefreshtimestamp_utc, '%Y-%m-%d %H:%i:%s')), TRY(date_parse(lastrefreshtimestamp_utc, '%m/%d/%y %H:%i'))) lastrefreshtimestamp_utc
   , accountid accountid
   , instancearn arn
   ...
   FROM
     compute_optimizer_ec2_instance_lines
   WHERE ((instancearn LIKE '%arn:%') AND (recommendationoptions_1_estimatedmonthlysavings_currency <> ''))
UNION    SELECT
     COALESCE(TRY(date_parse(lastrefreshtimestamp_utc, '%Y-%m-%d %H:%i:%s')), TRY(date_parse(lastrefreshtimestamp_utc, '%m/%d/%y %H:%i'))) lastrefreshtimestamp_utc
   , accountid accountid
   , instancearn arn
   ...
   FROM
     compute_optimizer_ec2_instance_lines
   WHERE ((instancearn LIKE '%arn:%') AND (recommendationoptions_2_estimatedmonthlysavings_currency <> ''))
UNION    SELECT
     COALESCE(TRY(date_parse(lastrefreshtimestamp_utc, '%Y-%m-%d %H:%i:%s')), TRY(date_parse(lastrefreshtimestamp_utc, '%m/%d/%y %H:%i'))) lastrefreshtimestamp_utc
   , accountid accountid
   , instancearn arn
   ...
   FROM
     compute_optimizer_ec2_instance_lines
   WHERE ((instancearn LIKE '%arn:%') AND (recommendationoptions_3_estimatedmonthlysavings_currency <> ''))
) 