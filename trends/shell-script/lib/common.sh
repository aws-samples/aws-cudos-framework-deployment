...
function check_existing_config() {
    ...
    deployFileName="work/$account/trends-create-dashboard.json"
    updateFileName="work/$account/trends-update-dashboard.json"
}
...
function CreateDatasets() {
  ...
  aws quicksight create-data-set --aws-account-id ${awsAccountId} --region ${AWS_DEFAULT_REGION} --cli-input-json file://work/${account}/datasets/daily_anomaly_detection.json
  aws quicksight create-data-set --aws-account-id ${awsAccountId} --region ${AWS_DEFAULT_REGION} --cli-input-json file://work/${account}/datasets/monthly_anomaly_detection.json
  aws quicksight create-data-set --aws-account-id ${awsAccountId} --region ${AWS_DEFAULT_REGION} --cli-input-json file://work/${account}/datasets/monthly_bill_by_account.json
}

function generate_queries() {
  [[ -d "work/$account/queries" ]] || mkdir -p "work/$account/queries"
  for i in `ls queries-templates`; do
    { echo "cat <<EOF"
